...
public sealed class TilePlacement : MonoBehaviour
{
    ...
    private float ComputeWall3DInnerRadius(HexCellView cell)
    {
        ...
        if (TryGetHexOuterRadiusFromSource(wall3DHexSource, out float outerR))
        {
            innerR = wall3DRadiusIsOuter ? outerR * COS30 : outerR;
        }
        else
        {
            if (cell != null && cell.rend != null)
            {
                var ext = cell.rend.bounds.extents;

                float baseR = Mathf.Max(ext.x, ext.z);
                float r = wall3DBoundsAreOuterRadius ? baseR * COS30 : Mathf.Min(ext.x, ext.z);
